...
class QAModel(nn.Module):
    def __init__(self, input_size, embedding_size, story_hidden_size, query_hidden_size, output_size, n_layers=1,
        ...
        self.story_embedding = nn.Embedding(input_size, embedding_size)  # Embedding bildet ab von Vokabular (Indize)
        #  auf n-dim Raum
...
# Train cycle
def train(model, train_loader, criterion, optimizer, epoch, start):
    total_loss = 0
    correct = 0
    ...
    print('\nTraining set: Accuracy: {}/{} ({:.0f}%)\n'.format(correct, train_data_size, accuracy))
...
def test(model, test_loader, criterion):
    ...
    print('\nTest set: Accuracy: {}/{} ({:.0f}%)\n'.format(correct, test_data_size, accuracy))
